  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#111827] p-4">
      <div className="bg-[#18181B] border border-zinc-800 text-white rounded-2xl shadow-2xl p-8 w-full max-w-md transition-all duration-500">
        ...
        {step === 'form' && category === 'crypto' && (
          <form className="space-y-4" onSubmit={handleCryptoSubmit}>
            ...
            <div className="flex gap-4 mt-4">
              <button type="submit" className="flex-1 py-2 rounded bg-accent text-white font-bold hover:bg-brand transition">Submit</button>
              <button type="button" className="flex-1 py-2 rounded bg-zinc-800 text-white font-bold hover:bg-zinc-700 transition" onClick={handleReset}>Back</button>
            </div>
          </form>
        )}
        ...
        {step === 'form' && category === 'stock' && (
          <form className="space-y-4" onSubmit={handleStockSubmit}>
            ...
            <input
              ...
              onChange={e => setStockForm(f => ({ ...f, bought_price_aud: e.target.value }))}
              required
            />
            <input
              className="w-full border border-zinc-700 rounded p-2 bg-[#18181B] text-white"
        ...
        {step === 'form' && category === 'super' && (
          <form className="space-y-4" onSubmit={handleSuperSubmit}>
            ...
            <input
              ...
              onChange={e => setSuperForm(f => ({ ...f, voluntary_contributions: e.target.value }))}
              required
            />
            <input
              className="w-full border border-zinc-700 rounded p-2 bg-[#18181B] text-white"
              ...
              onChange={e => setSuperForm(f => ({ ...f, employer_contributions: e.target.value }))}
              required
            />
            <input
              className="w-full border border-zinc-700 rounded p-2 bg-[#18181B] text-white"
        ...
        {step === 'result' && result && (
          <div className="flex flex-col items-center">
            <h2 className={`text-xl font-bold mb-4 ${result.success ? 'text-green-400' : 'text-red-400'}`}>{result.success ? 'Success!' : 'Error'}</h2>
            <p className="mb-6">{result.message}</p>
            <button className="py-2 px-6 rounded bg-accent text-white font-bold hover:bg-brand transition" onClick={handleReset}>Add Another</button>
          </div>
        )}
      </div>
    </div>
