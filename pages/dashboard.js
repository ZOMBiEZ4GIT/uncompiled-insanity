    ...
    <div className="w-full h-full min-h-[90vh] flex flex-col gap-8 items-center justify-center">
      ...
      <div className="grid grid-cols-4 gap-8 w-full" style={{ minHeight: 300 }}>
        {assetsData.map((asset, idx) => (
          <div
            key={idx}
            className="rounded-2xl shadow-lg p-8 bg-[#18181B] border border-zinc-800 text-white flex flex-col gap-2 items-center justify-between h-full"
          >
            <div className="flex items-center gap-2 mb-2">
              {assetIcons[asset.title]}
              <span className="font-bold text-lg text-accent drop-shadow">
                {asset.title}
              </span>
            </div>
            <AssetLineChart
